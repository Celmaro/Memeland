...
export class HyperliquidAdapter {
  ...
  // PvP leaderboard cache (stats-data response is large: ~41k traders / tens of MB) —
    // fetched at most once per hour. Module-level — shared across instances.
  private static leaderboardCache = new TtlCache<HyperliquidTrader[]>({ ttlMs: 60 * 60 * 1000 });
  ...
  public async fetchLeaderboardTraders(
    ...
    try {
        const cached = HyperliquidAdapter.leaderboardCache.get('board');
        if (!cached) {
          ...
          HyperliquidAdapter.leaderboardCache.set('board', traders);
          console.log(`[HYPERLIQUID] leaderboard cached: ${traders.length} traders (stats-data, 30d, TTL 1h)`);
        }
        return (HyperliquidAdapter.leaderboardCache.get('board') ?? []).slice(0, topN);
      } catch (err: unknown) {
